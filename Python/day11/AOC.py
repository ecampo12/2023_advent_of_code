...
def distance(start, end, empty_rows, empty_cols):
    manhattan_dist = abs(start[0] - end[0]) + abs(start[1] - end[1])

    # Additional distance for crossing empty rows and columns
    row_crossings = len([row for row in empty_rows if min(start[0], end[0]) < row < max(start[0], end[0])])
    col_crossings = len([col for col in empty_cols if min(start[1], end[1]) < col < max(start[1], end[1])])
